...
client.once('ready', () => {
    ...
    cacheRoles(client.guilds.cache.get('138027449610010625'));
});
...
async function cacheRoles(guild) {
    const rolePersistCache = await guild.members.fetch();
    rolePersistCache.map((value, key) => {
        rolePersistCache.delete(key);
        rolePersistCache.set(key, value.roles.cache);
    });
}