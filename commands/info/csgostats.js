...
module.exports = class CSGOSTATSCommand extends Command {
  ...
  async run(message, { user }) {
    ...
    // creating browser
    csgoEmbed.description = `\`${progressBar(
      8,
      20,
    ...
    try {
      ...
      csgoEmbed.description = oneLine`
            ...
            `;
      msg.edit(csgoEmbed);
      await page.goto(`https://csgostats.gg/player/${steamID}#/live`, {
        waitUntil: "networkidle0",
      });

      // click on check live game button
      const [button] = await page.$x(
        "//button[contains(., 'Check for live game')]",
      );
      if (button) {
        await button.click();
      }
      ...
      console.error(error);
      csgoEmbed.description =
        "An error occurred retrieving your csgostats page";
      msg.edit(csgoEmbed);
      browser.close();
      return;
    }
    ...
    if (imageHeight === 22) {
      ...
      await message.channel.send({
        files: ["./assets/images/csgostats.png"],
        embed: returnEmbed,
      });
    }
