...
module.exports = class DealsCommand extends Command {
  ...
  async run(message, { game }) {
    ...
    let inMsg;
    try {
      inMsg = await message.channel.awaitMessages(
    ...
    list.delete();
    const choice = parseInt(inMsg.first().content);
    inMsg.first().delete();
    ...
    response = await fetch(url).catch((err) => console.error(err));
      ...
      if (gameData.price && gameData.lowest) {
        messages.push(
          message.say(stripIndents`
            ${oneLine`On sale? ${gameData.price.cut !== 0 ? "Yes" : "No"} (${
              gameData.price.cut
            }% off)`}
            ${oneLine`Current Best Price: 
              ${gameData.price.price_formatted} 
              (${gameData.price.store})
            `}
            Link: ${gameData.price.url}

            ${oneLine`Historical Low Price: 
              ${gameData.lowest.price_formatted} 
              (${gameData.lowest.cut}% off) 
              (${gameData.lowest.store})
            `}
